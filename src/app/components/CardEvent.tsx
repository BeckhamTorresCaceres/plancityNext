...
export default function CardEvent({ event, categoryName = "General", isFavorite = false, onFavorite }: CardEventProps) {
  return (
    <div className="theme-card bg-card w-full mx-auto h-full rounded-3xl overflow-hidden border border-theme shadow-2xl flex flex-col hover:scale-[1.01] group">
      
      <div className="relative aspect-[16/10] w-auto mx-3 mt-3 bg-card-inner rounded-2xl overflow-hidden theme-transition">
        <img 
        ...
        {categoryName && (
          <span className="absolute top-3 left-3 bg-black/40 backdrop-blur-md text-white text-xs font-medium px-3 py-1 rounded-full uppercase tracking-wider border border-white/10">
            {categoryName}
          </span>
        )}
      ...
      <div className="px-5 pb-5 pt-4 flex flex-col flex-1 text-fg justify-between theme-transition">
        <div>
          <h3 className="text-lg font-bold tracking-tight mb-3 line-clamp-1">{event.name}</h3>
          
          <div className="space-y-1.5 text-xs sm:text-sm text-muted mb-4 theme-transition">
            <div className="flex items-center gap-2">
              <MapPin className="w-4 h-4 text-muted-2 shrink-0" />
              <span className="truncate">{event.location}</span>
            </div>
            <div className="flex items-center gap-2">
              <Calendar className="w-4 h-4 text-muted-2 shrink-0" />
              <span>{event.date}</span>
            </div>
          </div>
        </div>

        <div className="border-t border-theme pt-4 flex items-center justify-between theme-transition">
